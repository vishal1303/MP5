...
def plot_line_graph(graphs_path,keys,fname,title,y_label,xaxis="Bandwidth (Gbps)", xtransform=None):
  ...
  for key in keys:
    ...
    y = []
    if i == 0:
      line1, = plt.plot([0],'r:', label="Ideal", linewidth=10)
      line2, = plt.plot([0],'rs-', label="MP5 (simulation)", linewidth=2)
      line3, = plt.plot([0],'bd-', label="MP5 (prototype)", linewidth=2)
...
plot_line_graph(load_path,Keys,["results_heavytail_1.dat","results_heavytail_0.dat","prototype.dat"],"sequencer","Normalized Throughput",xaxis="Number of Pipelines", xtransform=None)
